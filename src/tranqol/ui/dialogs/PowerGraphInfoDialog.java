...
public class PowerGraphInfoDialog extends BaseDialog{
    ...
    public PowerGraphInfoDialog(){
        ...
        cont.table(modes -> {
            modes.button("@tq-power-info.producer", () -> {
                currType = PowerInfoType.producer;
                refresh();
            }).growX();
            modes.button("@tq-power-info.consumer", () -> {
                currType = PowerInfoType.consumer;
                refresh();
            }).growX();
            modes.button("@tq-power-info.battery", () -> {
        ...
        cont.pane(p -> infoTable = p.table().grow().top().get()).grow().top();
    ...
    private void refresh(){
        ...
        switch(currType){
            case producer -> {
                ...
                prodKeys.sort();
                prodKeys.each(id -> {
                    infoTable.add(new PowerInfoGroup(producers.get(id), PowerInfoType.producer, opened.contains(id), collToggled)).growX().top();
                    infoTable.row();
                });
            }
            case consumer -> {
                ...
                consKeys.sort();
                consKeys.each(id -> {
                    infoTable.add(new PowerInfoGroup(consumers.get(id), PowerInfoType.consumer, opened.contains(id), collToggled)).growX().top();
                    infoTable.row();
                });
            }
            case battery -> {
                ...
                battKeys.sort();
                battKeys.each(id -> {
                    infoTable.add(new PowerInfoGroup(batteries.get(id), PowerInfoType.battery, opened.contains(id), collToggled)).growX().top();
                    infoTable.row();
                });
            }
        }
